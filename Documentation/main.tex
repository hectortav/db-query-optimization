...
\begin{itemize}
    \item IntermediateArray consists of the row ids of each array (included in the currently handled predicate) that took part in previous joins. Each column is identified by the corresponding predicate array’s id.
...
\begin{itemize}
    ...
    \item Parallelization increases the speed of the program significantly because the execution load is split so that each thread is handling a different Job at each given moment.
